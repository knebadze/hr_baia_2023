...
class EnrollmentAgreeRepository
{
    function agree($data) {
        ...
        try {
            ...
            if ($data['enrollment_type'] == 2) {
                ...
                $this->updateRegisterLog($data['candidate_id'], $data['type'], $data['money']);
    ...
    function updateRegisterLog($candidate_id, $type, $money) {
        $candidate = Candidate::where('id', $candidate_id)->first();
        $register = userRegisterLog::where('user_id', $candidate->user->id)->first();
        if ($type == 0) {
            //არასრული ჩარიცხვა
            $register->update([
    ...
    function addHrBonus($type, $id, $bonus) {
        try {
            ...
            if ($type == 2) {
                ...
                $salary->update([
                    'hr_bonus_from_paid_registration' => $salary->hr_bonus_from_paid_registration + $bonus,
                    'full_salary' => $salary->full_salary + $bonus
                ]); 
            }
            return $salary;
