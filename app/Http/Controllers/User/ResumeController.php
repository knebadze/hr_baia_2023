    ...
    public function index()
    {
        ...
        $candidate = Candidate::where('user_id', $auth->id)
                    ->with([
                        ...
                        'generalWorkExperience', 'characteristic', 'familyWorkExperience'
                    ])->first();
        ...
        $workInformation = WorkInformation::where('candidate_id', $candidate->id)->with(['category', 'workSchedule','currency'])->get()->toArray();
        $data = [
           'candidate' => $candidate,
           'user' => $user,
           'yesNo' => $yesNo,
