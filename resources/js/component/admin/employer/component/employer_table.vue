        ...
        const employerDelete = async (id) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    let res = await axios.post("/employer_delete/" + id);
                    if (res.status == 200) {
                        if (res.data.type == "e") {
    ...
    methods: {
        ...
        async blackListModal(id) {
            if (!this.blackListComponent) {
                let module = await import(
                    "../../candidate/modal/add_black_list.vue"
                );
        ...
        async openSendMessageModal(item) {
            ...
            if (!this.sendMessageComponent) {
                let module = await import(
                    "../../candidate/modal/send_message_modal.vue"
                );
