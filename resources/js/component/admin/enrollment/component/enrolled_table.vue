        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <tr v-for="(item, index) in items" :key="index">
                    <td><span :class="(item.enrollment_type == 1)?'badge badge-primary':'badge badge-info'">{{ (item.enrollment_type == 1)?'კანდიდატი':'ვაკანსია' }}</span></td>
                    ...
                    <td>{{ (item.who_is_counting == 1 )?'კანდიდატი':'დამსაქმებელი' }}</td>
                    <td>{{ item.name }}</td>
                    <td><span :class="(item.type == 1)?'badge badge-success':'badge badge-danger'">{{ (item.type == 1)?'სრული':'არასრული' }}</span></td>
                    <td>{{ item.money }}</td>
                    <td>{{ item.hr_percent }}</td>
                    <td>{{ item.hr_bonus }}</td>
                    <td>{{ item.created_at }}</td>
                    <td>
                        <button type="button" class="btn btn-info" @click="showFile(item.file_path)" title="" :disabled="(item.file_path)?false:true"><i class="fa fa-eye"></i></button>
                    </td>
                    <td v-if="role_id == 1">
