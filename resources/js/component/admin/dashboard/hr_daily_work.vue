...
const items = computed(() =>
    props.role_id === 1 ? props.data.hr : props.data
);
...
const totalVacancy = computed(() =>
    _.sumBy(items.value.dailyWork, (item) => item.has_vacancy)
);
const totalApprove = computed(() =>
    _.sumBy(items.value.dailyWork, (item) => item.approved_by_employer)
);
const totalProbation = computed(() =>
    _.sumBy(items.value.dailyWork, (item) => item.has_probationary_period)
);
const totalEmployed = computed(() =>
    _.sumBy(items.value.dailyWork, (item) => item.employed)
);
const totalEnrollmentVacancy = computed(() =>
    _.sumBy(items.value.dailyWork, (item) => item.has_enrollment_vacancy)
);
const totalEnrollmentCandidate = computed(() =>
    _.sumBy(items.value.dailyWork, (item) => item.has_enrollment_register)
);
const totalRegistered = computed(() =>
    _.sumBy(items.dailyWork, (item) => item.candidate_has_registered)
);
</script>
<template lang="">
    <div class="card direct-chat direct-chat-primary">
        <div class="card-header">
            <h3 class="card-title">
                {{ role_id == 1 ? "ჰრ_ის" : "" }} დღის შესრულებული სამუშაოს
                ცხრილი
            </h3>
        ...
        >
            <p class="text-success">
                <strong>ვაკანსია მიიღო: {{ items.lastRecord }}</strong>
            </p>
            <p class="text-warning">
                <strong>რიგშია: {{ items.nextRecord }}</strong>
            </p>
            <p class="text-danger">
                <strong
                    >არააქტიური:
