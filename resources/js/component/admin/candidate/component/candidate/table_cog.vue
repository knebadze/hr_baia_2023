<template lang="">
    <div class="dropdown">
        ...
        >
            ...
            <a
                v-if="
                    item.status_id != 10 && item.status_id != 12 && role_id != 4
                "
                class="dropdown-item"
                href="#"
                @click="addInVacancy(item)"
                >ვაკანსიაში დამატება</a
            >
            <a
                v-if="role_id != 4"
                class="dropdown-item"
                :href="attachedUrl + '/' + item.id"
                >მიბმული ვაკანსიები</a
            >
            <a class="dropdown-item" :href="relevantUrl + '/' + item.id"
                >შესაბამისი ვაკანსიები</a
            >
            <a
                v-if="
                    item.status_id == 11 ||
                    (item.status_id == 14 && role_id != 4)
                "
                class="dropdown-item"
                href="#"
                @click="scheduleModal(item.id)"
                >გრაფიკი</a
            >
            <a
                v-if="
                    item.user.register_log &&
                    item.user.register_log.creator_id == hrId &&
                    item.registration_fee != 1
                "
                href="#"
                class="dropdown-item"
                @click="enrolled(item.user)"
                >ჩარიცხა</a
            >
            <a
                v-if="
            ...
            >
            <a
                v-if="item.status_id == 9"
                class="dropdown-item"
                href="#"
                @click="blackListModal(item.id)"
                >შავ სიაში დამატება</a
            >
            <a
                v-if="role_id == 1"
    ...
    <component
        :is="sendMessageComponent"
        v-if="sendMessageComponent"
        :visible="showSendMessageModal"
        :item="modalItem"
    />
    <component
        :is="blackListComponent"
        v-if="blackListComponent"
