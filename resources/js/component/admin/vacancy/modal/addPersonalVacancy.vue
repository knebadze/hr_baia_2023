<script setup>
    ...
    const show = async() =>{
        try {
            const response = await getClassificatory('/get_add_personal_info', { candidate_id:props.item.candidate_id, vacancy_id:props.item.vacancy_id })
            const { data } = response
            cla.value = data.cla
            ...
            m.value.interview_place = !m.value.interview_place ? cla.value.interviewPlace.find( (o) => o.id == m.value.interview_place_id): m.value.interview_place;
    ...
    const filterCla = () =>{
        ...
        if(check && candidateInfo.value.this_vacancy.qualifying_type_id == 4){
            ...
            filterQualifyingType = cla.value.qualifyingType.filter(item => item.id >= candidateInfo.value.this_vacancy.qualifying_type_id && item.id != 5 && item.id < 7);
        }  else {
            filterQualifyingType = cla.value.qualifyingType.filter(item => item.id !== 5 && item.id !== 7 && item.id !== 8);
        }
        ...
        .then(function (response) {
            ...
            if (response.status == 200) {
                ...
                if (link == 'selection_personal') {
                    props.emitResponse( { id: props.item.candidate_id, bool:false });
                }else{
                    setTimeout(() => {
    ...
    /* Read more about isConfirmed, isDenied below */
            if (result.isConfirmed) {
                axios.post('/delete_vacancy_personal' ,{
                    data: {'candidate_id':props.item.candidate_id, 'vacancy_id':props.item.vacancy_id },
                })
                .then(function (response) {
                    // handle success
                    if (response.status == 200) {
          ...
          <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
              <div class="modal-content">
                ...
                <div class="modal-body">
                    <div v-if="candidateInfo && candidateInfo.hasOwnProperty('this_vacancy') && candidateInfo.this_vacancy" class="col-12">
                    ...
                    <div v-if="candidateInfo && candidateInfo.hasOwnProperty('another_vacancy') && candidateInfo.another_vacancy.length > 0" class="col-12">
                        <div  class="alert alert-dismissible alert-danger" >
                            <h5><i class="icon fas fa-info"></i> შეტყობინება!</h5>
                            <span>ეს კანდიდატი უკვე გყავთ დამატებული <u>სხვა</u> ვაკანსიაზე</span>
                            <ul class="border py-2">
                                <li v-for="(item, index) in candidateInfo.another_vacancy" :key="index"><u class="text-primary" @click="openModal(item.vacancy.id)">{{ item.vacancy.code }}</u> -- {{ item.qualifying_type.name }}</li>
                            </ul>
                ...
                <div class="modal-footer">
                    ...
                    <div v-else>
                        <div v-if="candidateInfo && candidateInfo.hasOwnProperty('this_vacancy') && candidateInfo.this_vacancy">
